...
#' 
#' @description Function to check for presence of DQ reports, both excel and html. Runs best when your working directory is registrydqchecks/R/
#'
...
check_for_dq_reports <- function(base_report_url, base_html_report_url, output_url, year_month = NULL)
{
  ...
  #Get custom year
  user_year <- str_sub(year_month, start = 0, end = 4)
  
  # Listing out the html report directories
  AA_dir <- glue("{base_html_report_url}/AA/DQ Checks/Reports/{user_year}/{year_month}/")
  AD_dir <- glue("{base_html_report_url}/AD/DQ Checks/Reports/{user_year}/{year_month}/")
  IBD_dir <- glue("{base_html_report_url}/IBD/DQ Checks/Reports/{user_year}/{year_month}/")
  MS_dir <- glue("{base_html_report_url}/MS/DQ Checks/Reports/ms/{user_year}/{year_month}/")
  NMO_dir <- glue("{base_html_report_url}/NMO/DQ Checks/Reports/{user_year}/{year_month}/")
  PSO_dir <- glue("{base_html_report_url}/PSO/DQ Checks/Reports/{user_year}/{year_month}/")
  RA_dir <- glue("{base_html_report_url}/RA/DQ Checks/Reports/{user_year}/{year_month}/")
  RAJ_dir <- glue("{base_html_report_url}/RA Japan/DQ Checks/Reports/raj/{user_year}/{year_month}/")
